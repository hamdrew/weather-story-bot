...
The lifecycle configuration SHALL delete a current image only when it is replaced; current images SHALL remain after story expiration. It SHALL expire noncurrent object versions after 30 days and expire uncommitted `staging/` objects after 7 days. The publisher SHALL not treat a noncurrent image version as immediately usable.
...
#### Scenario: A current image is no longer needed
- **WHEN** a newer image replaces it
- **THEN** the system removes the superseded current image while S3 retains any noncurrent version for no more than 30 days

#### Scenario: A staging upload is not committed
- **WHEN** an object remains under the `staging/` prefix without a committed current-story reference for 7 days
- **THEN** lifecycle management expires the staging object

### Requirement: Provision same-Region history recovery controls
The deployment SHALL enable DynamoDB point-in-time recovery with `RecoveryPeriodInDays` set to 35 and configure AWS Backup to create one monthly DynamoDB backup retained for one year. It SHALL enable DynamoDB TTL on one numeric Unix-epoch-seconds attribute for publication attempts, transition events, run results, quarantine records, and alert-fingerprint state, set 30 days after creation or latest update; `OFFICE#{office_id}/CURRENT` and current-story records omit that attribute. The deployment SHALL preserve S3 Versioning for the image bucket and SHALL NOT configure cross-Region replication for MVP. Runtime roles SHALL not delete retained office/story records or current images because of story expiration; the publisher MAY delete a current image only when replacing it. The documented deployment/operator recovery authority SHALL perform only authorized recovery and permanent-purge procedures.

The recovery runbook SHALL restore DynamoDB only to a new isolated table, reapply required IAM policies, tags, TTL, PITR, alarms, and other non-data configuration before a controlled cutover, and verify sampled current-story, attempt, transition, and run records. It SHALL disable schedules before a production recovery cutover, retain the source table until validation completes, and document rollback to the source table. The system SHALL perform and record this same-Region restore exercise quarterly.
...
The system SHALL obtain Telegram credentials through a pre-created, exact-ARN Secrets Manager secret rather than embedding them in source code, deployment configuration, or CloudFormation outputs. The secret `SecretString` SHALL be JSON with exactly `schema_version` (integer `1`) and non-empty `telegram_bot_token` (string); Telegram channel and private alert-recipient identifiers SHALL remain non-secret runtime configuration. The publisher composition root SHALL load the packaged, versioned enriched office registry and environment configuration from configured read-only file paths, validate them before processing an invocation, and receive only exact environment-scoped `HISTORY_TABLE_NAME`, `IMAGE_BUCKET_NAME`, `ALERT_TRIGGER_TOPIC_ARN`, and `TELEGRAM_SECRET_ARN` resource references. It SHALL create bounded-timeout NWS/image clients and DynamoDB/S3/SNS clients from those inputs; select a mock Telegram adapter for all dev message operations; and use the schema-validated, at-most-60-second cached `AWSCURRENT` secret only to construct the staging/prod live Telegram adapter. Publisher and alert-dispatcher roles SHALL have only `secretsmanager:GetSecretValue` permission on that exact secret ARN, restricted to `AWSCURRENT`, and SHALL not receive secret listing, write, rotation, or access to other secrets. If a customer-managed KMS key encrypts the secret, each role SHALL receive `kms:Decrypt` only for that key.

#### Scenario: Service starts in a deployed environment
- **WHEN** the scheduled service is invoked
- **THEN** it validates its packaged registry and environment configuration, constructs only its environment-scoped runtime dependencies, and accesses its required Telegram configuration without exposing the bot token in application source or operational logs
...
The publisher role SHALL access only this application's DynamoDB story, attempt, transition, run, and quarantine key families; it SHALL use `dynamodb:LeadingKeys` conditions on those explicit partition-key prefixes wherever supported. It SHALL have only the required `GetItem`, `PutItem`, `UpdateItem`, `Query`, and conditional/transactional write actions on the table and required indexes. It SHALL access only this bucket's `staging/` and current-image prefixes: list only those prefixes, read/write/copy objects as needed, and delete only `staging/` objects or superseded current image objects. It SHALL read only the exact Telegram secret at `AWSCURRENT`, publish application failure events only to the alert-trigger SNS topic, and write only its own logs and metrics.
...
The office-info role SHALL read/write only this environment's `OFFICE#{office_id}/CURRENT` records, read the exact `AWSCURRENT` secret, call the NWS office/region endpoints, and use only the Telegram channel-management actions required to get/create/revoke the configured invite link, send/edit/pin the one managed information message, and verify the chat. It SHALL not create office audit/snapshot records; access story, attempt, transition, run, quarantine, alert, S3, or SNS resources; or invoke the publisher. Invite links, tokens, and raw Telegram payloads SHALL be excluded from logs.
...
#### Scenario: The first AWS application resource is requested

- **WHEN** a deployment mechanism would create the first AWS application resource for an environment
- **THEN** it verifies a current Infracost pass or documented authorized override for the exact template revision and environment, and blocks before the AWS apply path when that evidence is unavailable or invalid
