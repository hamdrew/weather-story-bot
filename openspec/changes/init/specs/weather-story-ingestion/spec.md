...
The system SHALL produce one durable summary for the single active `office_id` processed by each invocation. The summary SHALL include collection result; counts of stories discovered, published or edited, skipped, deferred, quarantined, rejected, and ambiguous; elapsed time; and final status (`success`, `success_with_deferred`, `success_with_quarantined_items`, or `failed`). Collection failures and unresolved selected rejected, ambiguous, or image-invalid publication outcomes SHALL make the durable run `failed`, set `required_work_completed=false`, and trigger the existing metrics and alert workflow. After persisting any run result, including `failed`, the handler SHALL return normally; it SHALL return an invocation failure only when it cannot persist the required run outcome or encounters an unhandled internal failure. Valid empty collections, normal skips, controlled deferrals, and successfully processed collections that quarantine malformed items SHALL return normally after their result is persisted.
...
- **THEN** the summary increments the relevant outcome count, persists `failed` with `required_work_completed=false`, invokes the existing alert workflow, and the Lambda returns normally after the durable result is written
