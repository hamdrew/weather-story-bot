...
- Maintain conditionally mutable current operational state for each office, including refreshed NWS-enriched metadata, active configuration, and managed pinned-message/invite references; use it to manage one nicely formatted pinned office-information message per real office channel through a separate on-demand Lambda.
...
- Establish the public GitHub remote, baseline pull-request checks, and default-branch protection before continuing MVP implementation; after the initial bootstrap, all source and policy changes merge through squash-only pull requests by default, with a documented sole-maintainer exception for `@hamdrew`, and production deployments use only the approved GitHub OIDC workflow.
...
- Establish staged, network-independent property-based testing for deterministic state, validation, retry, sanitization, and resource-limit invariants before any deployment.
- Require Infracost to be fully configured as a fail-closed gate before any AWS application resource is deployed, including workstation bootstrap and CI verification stacks.
