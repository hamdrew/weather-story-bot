...
| Transition | `ATTEMPT#{attempt_id}` / `TRANSITION#{ordinal}` | Append-only event for an attempt state change; the latest ordinal derives final attempt state. | Immutable; `expires_at` is 30 days after transition. | Prior/resulting state, actor, times, lease, error class, reconciliation reason, allowlisted response metadata. | Implemented transition persistence and operator reconciliation (2.5–2.6). |
| `RUN` | `RUN#{run_id}` / `RESULT` and `DEFERRAL#{office_id}#{source_story_id}` | Immutable result for exactly one office invocation, plus immutable controlled-deferral children for unstarted work. | Immutable; `expires_at` is 30 days after completion or recording. | Office/run IDs, collection outcome, status, timestamps/elapsed time, required-work flag, bounded reasons, per-office and aggregate counts; deferral identity and reason. | Implemented (2.2, 3.5). |
...
| 3.1–3.5 | Telegram use of durable state, expiration handling, retries, and scheduled runs | Implemented |
